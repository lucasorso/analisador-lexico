...
public class Automato {
    ...
    /* Variáveis de classe */
    private ArquivosUtil arqUtil = new ArquivosUtil();
    ...
    private HashMap hashMapTokens = new HashMap<String, Integer>();
    private HashMap naoTerminais;
    private JTable token_jTable;
    private List<Tokens> listTokensTerminaisEncontrados = new ArrayList<Tokens>();
//    String expressaoRegularNumero = "^([+-]?(\\d+\\.)?\\d+)$";
    String expressaoRegularNumero = "(([1-9][0-9]*)|(0))([.,][0-9]+)?";

    /* Construtor do automato */
    public Automato(String codigo, JTable token_jTable) {
        this.vetorCharCodigo = (codigo + "$").toCharArray();
        this.hashMapTokens = arqUtil.adicionarHashMapTokens();
    ...
    private void inicioAutomato(int posicaoAtual) {
        ...
        try {
            ...
            if (listTokensTerminaisEncontrados.size() != 0) {
                lastToken = listTokensTerminaisEncontrados.get(listTokensTerminaisEncontrados.size() - 1);
            }
...
//        Sintatico sint = new Sintatico(listTokens, hashMapTokens, naoTerminais);
    ...
    private void geraToken(String token, int linha, String tokenGenerico) {
        ...
        listTokensTerminaisEncontrados.add(novoToken);
    }
